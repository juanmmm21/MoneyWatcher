//! Tipos y lógica comunes a todos los formatos de extracto.
//!
//! Un extracto es una rejilla de celdas: da igual que venga de un CSV o de una
//! hoja de Excel. Aquí vive todo lo que trabaja sobre esa rejilla —encontrar la
//! cabecera, mapear las columnas, interpretar las filas y contrastar el
//! resultado con el saldo del propio fichero—, y cada formato solo aporta las
//! celdas.
...
pub(super) const MAX_HEADER_SCAN: usize = 500;

/// Una fila del fichero con el número de línea (CSV) o de fila (Excel) en la
/// que aparece, para poder señalarla en la interfaz.
pub(super) type GridRow = (u64, Vec<String>);

/// De dónde salieron las celdas. La vista previa lo enseña para que el usuario
/// vea que la app ha entendido su fichero antes de guardar nada.
...
impl Candidate {
    /// Cuánto se parece a una tabla de movimientos de verdad. Sirve para elegir
    /// entre lecturas posibles del mismo fichero: qué delimitador acierta en un
    /// CSV, o qué hoja de un libro de Excel trae los movimientos.
...
/// Busca en las primeras filas una cabecera que se pueda mapear a un
/// movimiento, y se queda con las filas de datos que la siguen.
pub(super) fn find_header_in(rows: &[GridRow], source: StatementSource) -> Option<Candidate> {
    for (index, (line, row)) in rows.iter().take(MAX_HEADER_SCAN).enumerate() {
        if row.iter().filter(|cell| !cell.is_empty()).count() < 3 {
            continue;
        }

        // Una fila que no se deja mapear no descarta el fichero: en un informe
        // la tabla puede empezar cientos de líneas más abajo.
        ...
        // mezclaría movimientos de otra cuenta con los de la elegida, así que la
        // importación se queda en la primera tabla.
        let mut records: Vec<GridRow> = Vec::new();
...
pub(super) fn detect_date_order(candidate: &Candidate) -> DateOrder {
...
pub(super) fn build_preview(candidate: Candidate, order: DateOrder) -> StatementPreview {
